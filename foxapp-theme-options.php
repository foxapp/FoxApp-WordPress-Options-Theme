...
if ( ! function_exists( 'foxapp_get_option' ) ) {
	...
	function foxapp_get_option( $name, $default = false ) {
		...
		//$options = apply_filters( 'foxapp_af_get_option', get_option( $config['id'] ), $name ); // af - apply filters
...
function foxapp_get_google_fonts_list() {
	$default_list = array(
	...
	$fonts_lst = array();
	if ( false === ( $fonts_lst = get_transient( 'foxapp_admin_fonts_list' ) ) ) {
		...
		set_transient( 'foxapp_admin_fonts_list', $fonts_lst, $life_time );
	}
